\chapter{実験理論(担当:宮辺)}\label{theory}
...
であるので,
Cパリティの保存より,
パラポジトロニウムは偶数,オルソポジトロニウムは奇数の光子へのみ崩壊する.
ポジトロニウムの崩壊率は,
...
混合しできた新たなエネルギー固有状態を$\ket{+}$,$\ket{-}$とする.
...
図\ref{fig:plusstate},図\ref{fig:minusstate}より,
磁束密度が大きくなると,
$\ket{+}$に占める$\ket{0,0}$の割合は大きくなり,
$\ket{-}$に占める$\ket{0,0}$の割合は小さくなる.
...
図\ref{fig:pluslife},図\ref{fig:minuslife}より,
磁束密度が大きくなると,
$\ket{+}$の寿命$\tau_{+}$は短くなり,
$\ket{-}$の寿命$\tau_{-}$は長くなる.
本研究で用いる電磁石による磁場は 0.1 Tであり,
このときそれぞれの寿命は$\tau_{+} = 116.9 \si{ns}$,
$\tau_{-} = 125.0 \si{ps}$となり,
$\tau_{+}$は短くなるが,
$\tau_{-}$はほとんど変化しない.
本実験では磁場中での$\tau_{+}$を測定する.
